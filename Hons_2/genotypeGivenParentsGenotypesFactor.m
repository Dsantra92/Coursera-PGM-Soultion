...
%h from both parents and all possible genotypes for the child.
...
genotypeFactor.card = [allelesToGenotypes(end), allelesToGenotypes(end), allelesToGenotypes(end)];
...
for i = 1:size(genotypesToAlleles,1)
  for j = 1:size(genotypesToAlleles,1)
    par_1_alleles = genotypesToAlleles(i,:);
    par_2_alleles = genotypesToAlleles(j,:);    
    
    for p_1 = par_1_alleles
      for p_2 = par_2_alleles
        g = allelesToGenotypes(p_1, p_2);
        genotypeFactor.val(AssignmentToIndex([g,i,j,], genotypeFactor.card)) += 0.25;
      endfor
    endfor
